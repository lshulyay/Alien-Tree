...
EntityFruit = ig.Entity.extend({
	...
	//	this.healthValue = this.targetRadius / 5;
	...
	//	this.bounciness = ig.game.controller.randomFromTo(0.3,0.7);
	...
	clicked: function() {
		...
		//	ig.game.controller.healthMod(this.parentEntity,-this.healthValue * 2);
		}
	},
	...
	draw: function() {
		...
		ig.game.ctx.arc(this.pos.x + this.size.x / 2, this.pos.y + this.size.y / 2, this.radius, 0 , 2 * Math.PI, false);
		...
		// ig.game.ctx.fillStyle = 'rgba(255, 0, 0,' + this.alpha +')';
		ig.game.ctx.fill();
		...
		// ig.game.ctx.strokeStyle = 'rgba(139, 8, 8,' + this.alpha +')';
		ig.game.ctx.stroke();
		ig.game.ctx.restore();
...
EntityLeaf = ig.Entity.extend({
	...
	//	this.radgrad.addColorStop(0, 'rgba(85, 176, 5,' + this.alpha +')');
	//	this.radgrad.addColorStop(0.75, 'rgba(131, 255, 21,' + this.alpha +')');
