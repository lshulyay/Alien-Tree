	...
	update: function() {
		...
		if (ig.game.currentLevel === LevelMain) {
			...
			if (this.health === 0 && !ig.game.gameover) {
				...
				ig.game.spawnEntity(EntityTrigger, ig.system.width / 2 - 36, ig.system.height / 5 + 70 + 44, {kind: 'begin', size: {x: 70, y: 28}});
...
/*	clicked: function() {
	...
	depleteHealth: function() {
		if (this.healthDepletionTimer.delta() > 0 && ig.game.allTreeNodes.length > 0) {
	...
	damageTree: function(damage) {
		...
		//	console.log('depleting water ' + this.damagePerTick);
	...
	waterTree: function(amount) {
		...
		if (ig.game.allTreeNodes.length > 0) {
			this.water += amount;
		//	var node = null;
...
//	animSheet: new ig.AnimationSheet( 'media/gui/next.png', 300, 150 ),
	...
	checkHeat: function() {
		...
		else if (this.heatingCheck === 1 && this.parentEntity.touches(ig.game.pointer)) {
			this.heatingCheck = 0;
			this.heatLevel++;
	...
	draw: function() {
		...
		if (this.kind === 'trunk') {
			ig.game.ctx.moveTo(this.parentEntity.pos.x, this.parentEntity.pos.y);
			ig.game.ctx.lineTo(this.parentEntity.pos.x + this.parentEntity.size.x,this.parentEntity.pos.y);
			ig.game.ctx.lineTo(this.pos.x + this.size.x, this.pos.y);
			ig.game.ctx.lineTo(this.pos.x, this.pos.y);
			...
			if (this.parentEntity === null || this.health <= 0 && this.parentEntity.health > 0) {
				...
				// Draw main branch
				ig.game.ctx.moveTo(this.parentEntity.pos.x, this.parentEntity.pos.y);
				ig.game.ctx.lineTo(this.parentEntity.pos.x,this.parentEntity.pos.y + this.parentEntity.size.y);
				ig.game.ctx.lineTo(this.pos.x, this.pos.y + this.size.y);
				ig.game.ctx.lineTo(this.pos.x, this.pos.y);
