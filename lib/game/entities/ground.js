...
EntityGround = ig.Entity.extend({
    ...
    drawGrass: function() {
     ...
     /*       var x1 = 0;
            var y1 = this.pos.y;
        for (var i = 0; i < ig.system.width / 7; i++) {
            var x2 = x1 + 3;
            var y2 = y1 - 50;
            var curveX = x1 - 10;
            var curveY = y1 - 25;
            ...
            ig.game.ctx.strokeStyle = '#2c6e18';
            ig.game.ctx.fillStyle = '#4d9029';
            ig.game.ctx.lineWidth = 1;
            ig.game.ctx.moveTo(x1,y1);
            ig.game.ctx.quadraticCurveTo(curveX,curveY, x2, y2);
            ig.game.ctx.quadraticCurveTo(curveX + 6, curveY, x1 + 6, y1);
            ig.game.ctx.lineTo(x1,y1);
            ig.game.ctx.stroke();
            ig.game.ctx.fill();
            x1 += 7;
        } */
    ...
    //    context.strokeStyle = 'black';
     //   context.stroke(); */
      ...
      /*   var x1 = ig.system.width / 2;
        var y1 = ig.system.height / 2;
        var x2 = x1 + 2;
        var y2 = y1 - 50;
        ig.game.ctx.beginPath();
        ig.game.ctx.moveTo(x1, y1);
     //   ig.game.ctx.lineTo(x2, y2);
        ig.game.ctx.arcTo(x1 + 1, y1 - 20, x2, y2 - 20, 50);

      ig.game.ctx.lineWidth = 3;

      // set line color
      ig.game.ctx.strokeStyle = '#ffffff';
      ig.game.ctx.stroke(); */
