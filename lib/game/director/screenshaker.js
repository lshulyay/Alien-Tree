...
ig.EntityScreenshaker = ig.Entity.extend({
    ...
    update: function(){
        ...
        else if (this.resetTimer && this.resetTimer.delta() >= -1.5) {
            ig.game.screenShaking = false;
        }
    },
    draw: function(){
...
ig.ScreenShaker = ig.Class.extend({
    spring: new ig.EntityScreenshaker(),
    
    update: function(){
        if (this.timedShakeTimer){
            if (this.timedShakeTimer.delta() <= 0){
                var randomAngle = ig.game.controller.randomFromTo(0, Math.PI*2);
                ...
                ig.game.controller.quake.active = false;
            }
        }
        this.spring.update();
    },
