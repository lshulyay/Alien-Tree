...
MyGame = ig.Game.extend({
	...
	fruitDropSound: new ig.Sound('media/music/fruitdrop.*'),
	wateringSound: new ig.Sound('media/music/water.*'),
	branchCrackSound: new ig.Sound('media/music/branchcrack.*'),
	candleSound: new ig.Sound('media/music/candle.*'),


	controller: new ig.Controller(),
	screenShaker: new ig.ScreenShaker(),
	currentLevel: null,
	gravity: 1,
	...
	loadLevel: function( data ) {
		...
		this.gameover = false;
		ig.game.controller.acidRain = {chance: 2, active: false};
		ig.game.controller.snow = {chance: 2, active: false};
		ig.game.controller.quake = {chance: 2, active: false};
		if (this.currentLevel === LevelMain) {
			ig.game.controller.updateAllTreeNodes();
			this.coreSpeed = 0.5;
			this.spawnEntity(EntityTree, ig.system.width/2 - 25, ig.system.height - 125);
			...
			this.spawnEntity(EntityTool, ig.game.tree.pos.x - 255, ig.system.height - 200, {kind: 'water'});
			this.spawnEntity(EntityTool, ig.game.tree.pos.x - 305, ig.system.height - 200, {kind: 'fertilizer'});
			this.spawnEntity(EntityTool, ig.game.tree.pos.x - 365, ig.system.height - 200, {kind: 'candle'});
			this.spawnEntity(EntityTool, 0, 0, {kind: 'reloadlevel'});
			...
			this.spawnEntity(EntityTrigger, this.tree.pos.x + 50, ig.system.height / 2 + 54, {kind: 'jhowe', size: {x: 400, y: 20}});
			this.spawnEntity(EntityTrigger, this.tree.pos.x + 50, ig.system.height / 2 + 94, {kind: 'gameicons', size: {x: 300, y: 20}});
	...
	drawCredits: function() {
		if (this.tree) {
			...
			y += 35;
			x += 15;
			this.smallFont.draw( 'Jonathan Howe [music]', x, y, ig.Font.ALIGN.LEFT );
			y += 17;
			this.smallFont.draw( 'www.youtube.com/user/TodayIWillprobably', x, y, ig.Font.ALIGN.LEFT );
			y += 25;
			this.smallFont.draw( 'GameIcons [tool sprites]', x, y, ig.Font.ALIGN.LEFT );
			y += 17;
	...
	drawGameOver: function() {
		if (this.gameover) {
			...
			y += 70;
			this.mainFont.draw('You survived ' + this.currentYear + ' seasons...', x, y, ig.Font.ALIGN.CENTER);
			y += 25;
			this.mainFont.draw('and collected ' + this.fruitCollected + ' pieces of fruit', x, y, ig.Font.ALIGN.CENTER);
			y += 44;
			this.mainFont.draw(this.tip, x, y, ig.Font.ALIGN.CENTER);
			y += 44;
			ig.game.mainFont.draw('Again.', x, y, ig.Font.ALIGN.CENTER);
		}
	},
